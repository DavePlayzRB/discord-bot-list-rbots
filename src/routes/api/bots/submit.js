...
route.post("/:id", auth, async (req, res) => {
    ...
    try {
        await req.app.get('client').channels.cache.find(c => c.id === server.website_log_id).send(`<@${req.user.id}> ${resubmit ? "re" : ""}submitted <@${req.params.id}>: <@&${server.role_ids.bot_verifier}>`);
